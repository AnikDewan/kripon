...
export default function InsightsScreen() {
  ...
  const outflows = items.filter((item) => item.direction === 'debit');
  const total = outflows.reduce((sum, item) => sum + item.amountPaise, 0);
  const largest = [...outflows].sort((a, b) => b.amountPaise - a.amountPaise)[0];
  const sourceSpend = sources.map((source) => ({ source, amount: outflows.filter((item) => item.source === source).reduce((sum, item) => sum + item.amountPaise, 0) }));
  const categorySpend = Object.entries(outflows.reduce<Record<string, number>>((map, item) => ({ ...map, [item.category]: (map[item.category] ?? 0) + item.amountPaise }), {})).sort((a, b) => b[1] - a[1]);
  ...
  const weekdaySpend = weekdayLabels.map((label, index) => ({
    label,
    amount: Math.round(outflows.filter((item) => item.occurredAt.getDay() === index).reduce((sum, item) => sum + item.amountPaise, 0) / 100),
  }));
  const highDay = [...weekdaySpend].sort((a, b) => b.amount - a.amount)[0];
  const monthKeys = [...new Set(outflows.map((item) => `${item.occurredAt.getFullYear()}-${String(item.occurredAt.getMonth()).padStart(2, '0')}`))].sort().slice(-6);
  const monthlySpend = monthKeys.map((key) => {
    const [year, month] = key.split('-').map(Number);
    return {
      label: monthLabel(year, month),
      amount: Math.round(outflows.filter((item) => item.occurredAt.getFullYear() === year && item.occurredAt.getMonth() === month).reduce((sum, item) => sum + item.amountPaise, 0) / 100),
    };
  });
  ...
  const monthlyBudgetPace = monthlyBudget ? Array.from({ length: daysElapsed }, (_, index) => {
    const day = index + 1;
    runningSpend += outflows.filter((item) => item.category !== 'Transfers' && item.occurredAt.getFullYear() === now.getFullYear() && item.occurredAt.getMonth() === now.getMonth() && item.occurredAt.getDate() === day).reduce((sum, item) => sum + item.amountPaise, 0);
