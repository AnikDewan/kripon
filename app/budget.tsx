...
export default function BudgetScreen() {
  ...
  const previewLimit = amountToPaise(amount) ?? 0;
  ...
  return (
    <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: '#F7F6F1' }}>
      <ScrollView className="bg-paper" contentContainerStyle={{ gap: 24, paddingHorizontal: 20, paddingBottom: 32, paddingTop: 16 }} keyboardShouldPersistTaps="handled">
        <View className="flex-row items-center justify-between"><Text className="text-[28px] font-extrabold tracking-[-1.1px] text-ink">Set budget</Text><Pressable onPress={() => router.back()} accessibilityLabel="Cancel" className="h-10 w-10 items-center justify-center rounded-full bg-white active:opacity-70"><X size={20} color="#112A3D" strokeWidth={2} /></Pressable></View>
        ...
        <View className="gap-2"><Text className="text-[12px] font-bold text-ink">Limit</Text><View className="flex-row items-center rounded-2xl border border-line bg-white px-4"><Text className="mr-2 text-[22px] font-bold text-ink">₹</Text><TextInput value={amount} onChangeText={(input) => setAmount(sanitizeAmountInput(input))} keyboardType="decimal-pad" inputMode="decimal" placeholder="10,000" placeholderTextColorClassName="accent-ink-muted" selectionColorClassName="accent-teal" cursorColorClassName="accent-teal" className="flex-1 py-4 text-[24px] font-extrabold text-ink focus:border-teal" /></View></View>
